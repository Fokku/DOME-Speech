...
class IdentificationServiceHttpClientHelper:
    ...
    _IDENTIFICATION_PROFILES_URI = '/speaker/verification/v2.0/text-independent/profiles'
    _IDENTIFICATION_URI = '/speaker/identification/v2.0/text-independent/profiles/identifySingleSpeaker'
    _SUBSCRIPTION_KEY_HEADER = 'Ocp-Apim-Subscription-Key'
    _CONTENT_TYPE_HEADER = 'Content-Type'
    _JSON_CONTENT_HEADER_VALUE = 'application/json'
    _STREAM_CONTENT_HEADER_VALUE = 'audio/wav'
    ...
    def get_all_profiles(self):
        ...
        try:
            ...
            if res.status == self._STATUS_OK:
                ...
                return [IdentificationProfile.IdentificationProfile(profiles_raw['profiles'][i]) for i in range(0, len(profiles_raw))]
            else:
    ...
    def create_profile(self, locale):
        ...
        try:
            ...
            if res.status == self._STATUS_CREATED:
                # Parse the response body
    ...
    def identify_file(self, file_path, test_profile_ids, force_short_audio = False):
        ...
        try:
            ...
            request_url = '{0}?profileIds={1}&?ignoreMinLength={2}'.format(
                self._IDENTIFICATION_PROFILES_URI,
                urllib.parse.quote(test_profile_ids_str),
                force_short_audio)
            request_url = \
                "https://westus.api.cognitive.microsoft.com/speaker/identification/v2.0/text-independent/profiles/identifySingleSpeaker?profileIds={0}&ignoreMinLength={1}".format(
                    test_profile_ids_str,
                    force_short_audio)
            # Prepare the body of the message
            with open(file_path, 'rb') as body:
                # Send the request
